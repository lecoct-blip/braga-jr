...
/**
 * Header de produção (.site-header). Substitui o par toolbar+SiteHeader do
 * mockup — a toolbar era chrome de revisão e não vai pra prod (README §9.3).
  ...
  return (
    ...
    >
      <nav className="site-header-inner" aria-label="Navegação principal">
        ...
        {/* Nav mobile — abas roláveis (visível só < 768px via CSS).
            Mantém navegação no mobile; hambúrguer fica p/ depois (README §9.4). */}
        <ul className="site-nav-mobile" aria-label="Navegação (mobile)">
          {TABS.map((t) => (
            <li key={t.id}>
              <Link
                href={t.href}
